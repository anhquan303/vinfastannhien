...
export default function ProductStripFullWidthV5({
  ...
  const go = (slugOrId) => {
    if (movedRef.current) { movedRef.current = false; return; } // vừa kéo -> không click
  ...
  return (
    ...
    >
      {products.map((p) => {
        ...
        return (
          <Box key={slug} sx={{ scrollSnapAlign: 'start', display: 'flex', justifyContent: 'center' }}>
            <Box
              role="link"
              tabIndex={0}
              onClick={() => go(slug)}
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); movedRef.current = false; go(slug); }
              }}
              draggable={false}
              ...
              sx={{
                ...
                textDecoration: 'none',
                color: 'inherit',
                '&:focus-visible': { boxShadow: '0 0 0 3px rgba(255,44,139,0.35)' },
                userSelect: 'none',
              }}
            >
              <Box
